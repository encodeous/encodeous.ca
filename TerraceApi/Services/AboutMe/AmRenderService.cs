...
public class AmRenderService
{
    ...
    private readonly SKColor hightlightColor = SKColor.Parse("#5B5003");
    ...
    private SKShader GradientBG = SKShader.CreateLinearGradient(
        ...
        new SKPoint(Width, Height),
        new [] { SKColor.Parse("#171615"), SKColor.Parse("#0f1014") },
        new float[] { 0, 1 },
        SKShaderTileMode.Repeat);

    public async Task DrawAboutMeAsync(SKSurface surface, SKImageInfo info)
    {
        ...
        await DrawStatsAsync(canvas, 100, 250);
        DrawGreeting(canvas, info);
        var smallStyle = consoleStyle.WithTextSize(30);
        canvas.DrawText($"{DateTime.UtcNow.ToString("O")}                                   .NET {Environment.Version} & Skia", new SKPoint(25, Height - 25 + smallStyle.TextSize / 2), smallStyle);
    }
    ...
    private void DrawTitle(SKCanvas canvas, float x, float y, string text)
    {
        ...
        {
            Shader = Skia.Stripe(30, hightlightColor)
        });
    ...
    private async Task DrawStatsAsync(SKCanvas canvas, float x, float y)
    {
        ...
        // canvas.DrawRect(x, y, 700, 500, new SKPaint()
        // {
        //     Shader = Skia.Stripe(40, mutedColor),
        //     IsDither = true
        // });
        DrawTitle(canvas, x, y, "github statistics");
        canvas.DrawText($"fetched {stats.LastUpdated.Humanize()}", 
            new SKPoint(x + 530, y + 70), consoleStyle.WithTextSize(20).WithAlignment(SKTextAlign.Right));
        float offset = 140;
        ...
        // DrawLine($"updated at {stats.LastUpdated}", x + 10, y, canvas, consoleStyle, ref offset);
    }
    ...
    private void DrawGreeting(SKCanvas canvas, SKImageInfo info)
    {
        ...
        canvas.DrawText(messages[Random.Shared.Next(messages.Length)], new SKPoint(1070, 170 + light.TextSize / 2), light.WithAlignment(SKTextAlign.Right));
        // canvas.RotateDegrees(3);
    ...
    {
        var emojiChar = StringUtilities.GetUnicodeCharacterCode(character, SKTextEncoding.Utf32);
        using (var emoji = SKFontManager.Default.MatchCharacter(emojiChar))
        {
